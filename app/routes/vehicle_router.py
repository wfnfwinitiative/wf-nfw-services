...
@router.patch("/{vehicle_id}", response_model=VehicleRead, dependencies=[Depends(require_roles(["ADMIN", "COORDINATOR"]))])
...
@router.delete("/{vehicle_id}", dependencies=[Depends(require_roles(["ADMIN", "COORDINATOR"]))])
...
@router.post("/activate/{vehicle_id}", dependencies=[Depends(require_roles(["ADMIN", "COORDINATOR"]))])
