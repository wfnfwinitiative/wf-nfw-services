...
class OpportunityAllocation(Base):
    ...
    opportunity_item_id = Column(
        BigInteger,
        ForeignKey(f"{SCHEMA}.opportunity_items.opportunity_item_id"),
        nullable=False,
    )
    hunger_spot_id = Column(
        BigInteger, ForeignKey(f"{SCHEMA}.hunger_spots.hunger_spot_id"), nullable=False
    )
